...
import { Loader2, Dog, Cat, User as UserIcon, PawPrint, X } from 'lucide-react';
...
export function ReportWizard({ open, onOpenChange }) {
  ...
  const onFiles = async (files) => {
    ...
    setPhotos((p) => [...p, ...compressed.map((data_url) => ({ data_url }))].slice(0, 6));
  };

  const submit = async () => {
    if (!user) {
      toast('Please sign in first', { description: 'You need an account to submit a report.' });
      onOpenChange(false);
      nav('/login');
      return;
    }
    ...
    try {
      ...
      toast.success('Report submitted', { description: data.matches?.length ? `${data.matches.length} possible matches found.` : 'We will alert you when we find matches.' });
      const id = data.case.case_id;
      reset();
      onOpenChange(false);
      nav(`/cases/${id}`);
    } catch (e) {
      toast.error('Could not submit report', { description: e?.response?.data?.detail || 'Please try again.' });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder={isPerson ? 'e.g. Alex' : 'e.g. Buddy'} data-testid="report-name-input" />
            </div>
            {!isPerson && (
              <div>
                <Label>Species</Label>
                <Input value={species} onChange={(e) => setSpecies(e.target.value)} placeholder="dog, cat, rabbit, bird..." data-testid="report-species-input" />
              </div>
            )}
            {!isPerson && (
              <div>
                <Label>Breed</Label>
                <Input value={breed} onChange={(e) => setBreed(e.target.value)} placeholder="e.g. labrador" data-testid="report-breed-input" />
              </div>
            )}
            <div>
              <Label>Color / Distinctive markings</Label>
              <Input value={color} onChange={(e) => setColor(e.target.value)} placeholder="e.g. brown with white paws" data-testid="report-color-input" />
            </div>
            <div>
              <Label>Age</Label>
              <Input value={age} onChange={(e) => setAge(e.target.value)} placeholder="approx." data-testid="report-age-input" />
            </div>
            <div>
              <Label>Gender</Label>
              <Input value={gender} onChange={(e) => setGender(e.target.value)} placeholder="male / female / unknown" data-testid="report-gender-input" />
            </div>
            <div className="col-span-2">
              <Label>Last seen (date & time)</Label>
              <Input type="datetime-local" value={lastSeenAt} onChange={(e) => setLastSeenAt(e.target.value)} data-testid="report-last-seen-input" />
            </div>
          </div>

          <div>
            <Label>Description *</Label>
            <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe what happened, distinctive features, collar, tag, temperament, medical needs…" rows={4} required data-testid="report-description-input" />
          </div>

          <div>
            <Label>Where? (pin the location)</Label>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mt-3">
                {photos.map((p, i) => (
                  <div key={i} className="relative aspect-square rounded-md overflow-hidden bg-muted">
                    <img src={p.data_url} alt={`photo-${i}`} className="h-full w-full object-cover" />
                    <button type="button" onClick={() => setPhotos((arr) => arr.filter((_, j) => j !== i))} className="absolute top-1 right-1 h-6 w-6 rounded-full bg-black/60 text-white grid place-items-center">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
        ...
        <DialogFooter className="mt-6">
          ...
          <Button onClick={submit} disabled={!canSubmit || submitting} data-testid="report-wizard-submit-button">
